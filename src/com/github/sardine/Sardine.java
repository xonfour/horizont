...
public interface Sardine {
	 ...
	 * <p/>
	 * If the server also supports gzip compression, it should serve the contents in compressed gzip format and include "gzip" as the Content-Encoding. If the
	 * content encoding is present, Sardine will automatically decompress the files upon reception.
	 */
	void enableCompression();

	/**
	 * Send a <code>Basic</code> authentication header with each request even before 401 is returned. Uses default ports: 80 for http and 443 for https
	 *
	 ...
	 * @param url Path to the resource including protocol and hostname
	 * @param depth The depth to look at (use 0 for single ressource, 1 for directory listing)
	 * @param allProp If allprop should be used, which can be inefficient sometimes; warning: no allprop does not retrieve custom props, just the basic ones
	 * @return List of resources for this URI including the parent resource itself
	 * @throws IOException I/O error or HTTP response validation failure
	 */
	...
	/**
	 * Put an exclusive write lock on this resource. A write lock must prevent a principal without the lock from successfully executing a PUT, POST, PROPPATCH,
	 * LOCK, UNLOCK, MOVE, DELETE, or MKCOL on the locked resource. All other current methods, GET in particular, function independently of the lock.
	 * <p/>
	 * A WebDAV compliant server is not required to support locking in any form. If the server does support locking it may choose to support any combination of
	 * exclusive and shared locks for any access types.
	 *
	 * @param url Path to the resource including protocol and hostname
	 * @return The lock token to unlock this resource. A lock token is a type of state token, represented as a URI, which identifies a particular lock. A lock
	 *         token is returned by every successful <code>LOCK</code> operation in the lockdiscovery property in the response body, and can also be found
	...
	/**
	 * Uses <code>PUT</code> to send data to a server with a specific content type header. Repeatable on authentication failure.
	 *
	 * @param url Path to the resource including protocol and hostname
	 * @param data Input source
	...
	/**
	 * Uses <code>PUT</code> to upload file to a server with specific contentType. Repeatable on authentication failure.
	 *
	 * @param url Path to the resource including protocol and hostname
	...
	/**
	 * Uses <code>PUT</code> to send data to a server with a specific content type header. Not repeatable on authentication failure.
	 *
	 * @param url Path to the resource including protocol and hostname
	 * @param dataStream Input source
	 * @param contentType MIME type to add to the HTTP request header
	...
	/**
	 * A LOCK request with no request body is a "LOCK refresh" request. It's purpose is to restart all timers associated with a lock. The request MUST include
	 * an "If" header that contains the lock tokens of the locks to be refreshed (note there may be multiple in the case of shared locks).
	 *
	 * @param url Path to the resource including protocol and hostname
	...
	/**
	 * Releasing any resources that might be held open. This is an optional method, and callers are not expected to call it, but can if they want to explicitly
	 * release any open resources. Once a client has been shutdown, it should not be used to make any more requests.
	 */
