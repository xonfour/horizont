...
/**
 * Class used as a wrapper over the DropBox API's unchecked exceptions when an {@link IOException} is needed
 *
 * <p>
 * The problem mainly comes from {@link DbxClient.Uploader} and {@link DbxClient.Uploader}. Both of these methods define a {@code close()} method but none of
 * them implement {@link Closeable}. Worse than that, at least as far as the uploader is concerned, this method is not even idempotent, and all exceptions it
 * throws are <strong>unchecked</strong>.
 * </p>
 *
 * <p>
 * The file system API, however, needs "correct" {@link IOException}s to be thrown. We therefore capture all {@link RuntimeException}s thrown by either a
...
public final class DropBoxIOException extends IOException {
	...
	private static final long serialVersionUID = -2025761852367493133L;
	...
	/**
	 * Constructs an {@code IOException} with the specified cause and a detail message of {@code (cause==null ? null : cause.toString())} (which typically
	 * contains the class and detail message of {@code cause}). This constructor is useful for IO exceptions that are little more than wrappers for other
	 * throwables.
	 *
	 * @param cause The cause (which is saved for later retrieval by the {@link #getCause()} method). (A null value is permitted, and indicates that the cause
	 *        is nonexistent or unknown.)
